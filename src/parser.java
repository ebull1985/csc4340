...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Mon Mar 27 02:43:31 EDT 2017
  */
...
class CUP$parser$actions {
    ...
    //Unmatched columns
    HashMap<Column, String[]> unmatchedCols = new HashMap<Column, String[]>();
      ...
      switch (CUP$parser$act_num)
		...
		String a = (String)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		  String[] tref = new String[2];
            tref[0] = new String(t);
            tref[1] = new String(a);
            if (aliasTables.containsKey(tref[1])) semantics.add("Alias " + tref[1] + " name collision.");
		...
		SQLNode s = (SQLNode)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		  System.out.println("No Syntax Error");
            ...
            //Resolve unmatched cols
            ArrayList<String[]> allTNames = new ArrayList<String[]>();
            ...
            while (true) {
                ...
                curSN = bucket.pop();
                allTNames.addAll(curSN.getRelations());
                allCols.addAll(curSN.getColumns());
                if(curSN.getWhereNodes() == null) break;
                for(WhereNode wn : curSN.getWhereNodes()) {
                    if(wn.getLeftOperandType() != null && wn.getLeftOperandType().equals("col")) {
                        allCols.add(wn.getLeftOperandCol());
                    }
                    if(wn.getRightOperandType() != null && wn.getRightOperandType().equals("col")) {
                        allCols.add(wn.getRightOperandCol());
                    }
            ...
            HashMap<Column, String[]> matches = new HashMap<Column, String[]>();
            for(Column col : allCols) {
                String cName = col.getName();
                String cPrefix = col.getPrefix();         
                if (cPrefix != null && aliasTables.containsKey(cPrefix)) {
                    cPrefix = aliasTables.get(cPrefix);
                }
                for(String[] tname : allTNames) {
                    String tName = tname[0];
                    String tAlias = tname[1];
                    if(cPrefix != null) {
                        if(cPrefix.equals(tName)) {
                            if (matches.containsKey(col) && (!matches.get(col)[0].equals(tName))) {
                                    throw new SemanticException("Ambiguous column: " + cPrefix + "." + cName);
                            }
                            if(!db.relationExists(tName)) throw new SemanticException("Table " + tName + " does not exist.");
                            if(!db.getRelation(tName).attributeExists(cName)) throw new SemanticException("Column " + cPrefix + "." + cName + " does not resolve for table " + tName + ".");
                            col.setPrefix(cPrefix);
                            matches.put(col, tname);

                        }
                    } else {
                        if(db.relationExists(tName)) {
                            if(db.getRelation(tName).attributeExists(cName)) {
                                if (matches.containsKey(col) && !matches.get(col)[0].equals(tName)) {
                                    throw new SemanticException("Ambiguous column: " + cName);
                                } else {
                                    cPrefix = tName;
                                    col.setPrefix(cPrefix);
                                    matches.put(col, tname);
                        ...
                        } else throw new SemanticException("Table " + tName + " does not exist.");
                    }
                }
                if(!matches.containsKey(col) && cPrefix != null) throw new SemanticException("Could not resolve " + cPrefix + "." + cName);
                else if(!matches.containsKey(col)) throw new SemanticException("Could not resolve "  + cName); 
            }
