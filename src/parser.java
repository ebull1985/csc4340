...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Sun Mar 26 21:13:22 EDT 2017
  */
		 ...
		 WhereNode whereNode = new WhereNode();
            ...
            switch (sr.getType()) {
                ...
                case "int":
                    ...
                    break;
                case "col":
                    whereNode.setRightOperandName(sr.getColValue().getName());
		 ...
		 SQLNode sqlNode = new SQLNode(o, s, r);
            for(Column col : s) {
                ...
                if(matchCount == 0) {
                    ...
                    semantics.add("Ambiguous Column Reference: " + aName + ".1");
                }
            }
            if(owc != null) sqlNode.setWhereNodes(owc);
            for(WhereNode wn : owc) if(wn.getNested()) nested = true;
            //I know this code is a bit redundant but short on time, don't want to integrate
            //This is basically doing the same thing but for cols from whereNodes
            //Every time a SQLNode is created, check its tables to see if that's where they resolve
                    ...
                    if(db.relationExists(tname)) {
                        ...
                        if(db.getRelation(tname).attributeExists(whereCol.getName())) {
                            if(whereCol.getPrefix() == null) {
                                ...
                                //Checking to see if the WhereNode's original SQLNode
                                //Contains this table. If not, we have found this table in another
                                //SQLNode and can say with confidence that it is co-related.
                                if (whereCol.getSN() != null && sqlNode != whereCol.getSN()) {
                                    ArrayList<String> whereRelations = new ArrayList<String>();
                                    for(String[] relation : whereCol.getSN().getRelations()) {
                                        whereRelations.add(relation[0]);
                                    }
                                    if (!whereRelations.contains(tname)) {
                                        coRelated = true;
                                    }
                                }
                            }
