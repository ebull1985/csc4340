...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Sun Mar 26 14:30:04 EDT 2017
  */
		...
		SQLNode s = (SQLNode)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		 WhereNode whereNode = new WhereNode();
            ...
            if(s.getColumns().size() == 1) {
                ...
                } else if(se.getType() == "str" || se.getType() == "num") {
                    ColTypeAssertions.put(s.getColumns().get(0), se.getType());
                }
            } else {
                throw new SemanticException("Type mismatch: Can not compare expression to more than a single column.");
            }
            RESULT = whereNode; 
		 ...
		 WhereNode whereNode = new WhereNode();
            ...
            switch (sl.getType()) {
                ...
                case "int":
                    ...
                    break;
                case "col":
                    whereNode.setLeftOperandName(sl.getColValue().getName());
                    break;
            }
            ...
            switch (sr.getType()) {
                ...
                case "int":
                    ...
                    break;
                case "col":
                    whereNode.setRightOperandValue(sr.getColValue().getName());
                    break;
            }
            //Type checking
            if ((sl.getType() == "num" || sl.getType() == "str") && sr.getType() == "col") {
                ColTypeAssertions.put(sr.getColValue(), sl.getType());
            } else if (sl.getType() == "col" && (sr.getType() == "num" || sr.getType() == "str")) {
                ColTypeAssertions.put(sl.getColValue(), sr.getType());
            } else if (sl.getType() == "col" && sr.getType() == "col") {
                ColColAssertions.put(sl.getColValue(), sr.getColValue());
            } else if (sl.getType() != sr.getType()) {
                throw new SemanticException("Type mismatch: " + sl.toString() + " " + sr.toString() + ".");
            }
            RESULT = whereNode; 
		 ...
		 SQLNode sqlNode = new SQLNode(o, s, r);
            for(Column col : s) {
                ...
                if(col.getPrefix() != null) {
                    ...
                    aPrefix = col.getPrefix();
                    if(aliasTables.containsKey(aPrefix)) {
                        aPrefix = aliasTables.get(aPrefix);
                        col.setPrefix(aPrefix);
                    }
                }
                int matchCount = 0;
                for(String[] tnames : r) {
                    ...
                    if(db.relationExists(tname)) {
                        if(aPrefix == null) {
                            ...
                            if (aPrefix.equals(tname) && db.getRelation(tname).attributeExists(aName)) {
                                matchCount++;
                            }
                        }
                    } else {
                        throw new SemanticException("Table does not exist: " + tname);
                    }
                }
                if(matchCount == 0) {
                    throw new SemanticException("Column not found: " + aName);
                } else if (matchCount > 1) {
                    throw new SemanticException("Ambiguous Column Reference: " + aName);
                }
                if (col.getPrefix() == null) {
